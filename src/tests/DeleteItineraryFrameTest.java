...
class DeleteItineraryFrameTest {
...
//		Instantiates 3 valid itineraries, two of them being on the same date. It is expected that if
//		two itineraries are instantiated on the same date, the user wants the latest one to remain.
		...
		Date date11 = null;
		try {
		    date11 = dateFormat11.parse("10/03/2022");
		} catch (ParseException e) {
...
//		Performs a delete operation on the repeated date
		admin.deleteItinerary(date11);
		
//		Creates a new array list containing the Itineraries that are expected to remain.
		List<Itinerary> itinerariesLocal = new ArrayList<>();
		itinerariesLocal.add(itinerary);
		itinerariesLocal.add(itinerary11);
		
//		Gets the generated array list made with DAO methods
		List<Itinerary> itinerariesDao = admin.getAllItinerary();
		
//		Compares the array lists. The test should succeed if the DAO methods are robust enough 
